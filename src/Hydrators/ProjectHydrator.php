...
abstract class ProjectHydrator
{
    ...
    {
        $stmnt = $db->prepare("SELECT `projects`.`id`, `projects`.`name`, `projects`.`client_id`, `projects`.`deadline`, `clients`.`name` AS 'client_name', `clients`.`logo` AS 'client_logo' FROM `projects` LEFT JOIN `clients` ON `projects`.`client_id` = `clients`.`id`;");
